        ...
        if (!animation.isDeleting) {
          ...
          if (animation.currentLine < currentTextSet.length) {
            ...
            if (animation.currentChar <= line.length) {
              ...
              setDisplayText((prev) => {
                const newText = [...prev];
                newText[animation.currentLine] = line.substring(
                  0,
            ...
            animation.timeoutId = setTimeout(() => {
              ...
              if (
                ...
                animation.currentLine < currentTextSet.length
              ) {
                animation.currentChar =
                  currentTextSet[animation.currentLine].length;
              } else {
                animation.currentChar = 0;
              }
              animateText();
            ...
            if (animation.currentChar > 0) {
              ...
              if (animation.currentLine >= 0) {
                ...
                animation.timeoutId = setTimeout(() => {
                  ...
                  animation.currentLine = 0;
                  animation.currentChar = 0;
                  animation.textSetIndex =
                    (animation.textSetIndex + 1) % textSets.length;
  ...
  return (
    <div className="relative flex items-center justify-center w-[375px]">
      <div className="relative">
        ...
        {/* Text container moved above input area */}
        <div className="absolute top-[20%] left-1/2 -translate-x-1/2 text-center min-h-[240px] w-[340px]">
          <div className="flex flex-col items-center space-y-2">
            {displayText.map((line, index) => (
              <h3
                key={index}
                className="text-[44px] font-semibold font-manrope leading-[1.0] min-h-[48px] bg-gradient-to-r from-[#082f49] to-[#0c4a6e] bg-clip-text text-transparent"
              >
                {line}
              </h3>
            ))}
        ...
        {/* Input and button container moved lower to accommodate text above */}
        <div className="absolute top-[68%] left-1/2 -translate-x-1/2 w-[90%] flex flex-col items-center">
          {/* Input and button wrapped in a fixed container */}
          <div className="flex flex-col items-center w-full gap-5">
            <div className="relative w-[75%]">
              <div className="absolute left-4 top-1/2 -translate-y-1/2 text-black font-medium">
                +
              </div>
              <input
                type="tel"
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`
                w-[72px] h-[72px] rounded-full font-semibold text-base
